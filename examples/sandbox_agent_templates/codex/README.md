# Codex CLI 沙箱模板

对齐 e2b 官方 [`codex`](https://e2b.dev/docs/agents/codex) 模板，基于
本仓库的 `agents-base` 模板叠加 [OpenAI Codex CLI](https://github.com/openai/codex)。

## 镜像内容

| 组件 | 版本 | 说明 |
| --- | --- | --- |
| 基础模板 | `agents-base` | 继承 e2bdev/base、Node.js、基础开发/排障工具、GitHub CLI 和前端脚手架 |
| `@openai/codex` | `latest` | OpenAI Codex CLI |

> 镜像内不内置任何 API key，认证信息通过沙箱创建时的 `envs` 注入。

## 模板继承说明

`qshell.sandbox.toml` 使用 `from_template = "agents-base"`，真实 base 来自已构建的
`agents-base` 模板。当前 Dockerfile 中的 `FROM scratch` 仅用于 qshell 解析 Dockerfile，
不会作为基础镜像拉取。

构建本模板前，请先构建并发布 `examples/sandbox_agent_templates/agents-base`。

## 构建（通过 qshell.sandbox.toml 幂等）

```bash
cd examples/sandbox_agent_templates/codex

# 首次构建 + 后续重建：命令完全一样
qshell sandbox template build --wait

# 忽略缓存强制完整重建
qshell sandbox template build --no-cache --wait
```

qshell 会从当前目录的 `qshell.sandbox.toml` 读取 `name` / `from_template` /
`dockerfile` / `cpu_count` / `memory_mb` 等字段，并按 `name` 定位模板
（环境内全局唯一）。首次构建和后续重建使用同一条命令；不同账号或环境也不需要
手动维护 `template_id`。

> 建议使用最新版 qshell 构建和发布模板。

CLI flag 优先于配置文件，可临时覆盖：

```bash
# 临时调大内存，不改 qshell.sandbox.toml
qshell sandbox template build --memory 4096 --wait
```

## 发布 / 查看 / 删除

以下命令会从当前目录 `qshell.sandbox.toml` 按 `name` 自动定位模板：

```bash
qshell sandbox template publish -y    # 发布
qshell sandbox template get           # 查看详情
qshell sandbox template unpublish -y  # 取消发布
qshell sandbox template delete -y     # 删除模板
```

## 在沙箱中使用

```python
from e2b import Sandbox
import os

sbx = Sandbox("codex", envs={"OPENAI_API_KEY": os.environ["OPENAI_API_KEY"]})

result = sbx.commands.run(
    "codex exec --full-auto --skip-git-repo-check "
    "'write a FastAPI hello world app'"
)
print(result.stdout)
```

```javascript
import { Sandbox } from 'e2b'

const sbx = await Sandbox.create('codex', {
  envs: { OPENAI_API_KEY: process.env.OPENAI_API_KEY },
})

const res = await sbx.commands.run(
  `codex exec --full-auto --skip-git-repo-check "write a hello world in python"`
)
console.log(res.stdout)
```

### 沙箱下常用的 Codex CLI flag

| Flag | 说明 |
| --- | --- |
| `--full-auto` | 跳过审批交互，沙箱隔离环境下安全 |
| `--skip-git-repo-check` | 跳过 git 仓库归属检查 |
| `--json` | 输出 JSONL 事件流，便于程序消费 |
| `--output-schema` | 以 JSON Schema 约束输出格式 |

## 与官方 e2b `codex` 模板的差异

| 维度 | e2b 官方 | 本模板 |
| --- | --- | --- |
| 基础来源 | `e2bdev/base` | 通过 `agents-base` 继承 |
| Codex CLI | ✅ | ✅ |
| git / ripgrep / vim | ✅ / ⚠️ 未确认 / ❌ | ✅ / ✅ / ✅ |
| 常用排障工具 | ❌ | ✅（jq / less / tree / zip / unzip / procps / lsof / nc / dig / ping） |
| GitHub CLI | ❌ | ✅ |
| 前端脚手架 | ❌ | ✅（pnpm / tsx / vite） |

公共工具由 `agents-base` 统一维护，本模板 Dockerfile 只包含 Codex CLI 的增量安装。
